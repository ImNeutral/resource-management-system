...
<%@ attribute name="showProjects" required="true" type="java.lang.Boolean"%>
...
<table class="table table-bordered table-hover">
	...
	<tbody>
		...
		<c:forEach items="${ employee.projectEmployee }" var="projectEmployee">
			 <c:if test = "${ projectEmployee.active == active}">
			 	<tr id="project_member_details" data-id="${ projectEmployee.id }">
					<th scope="row">${ count }</th>
					
					<c:if test = "${ showProjects != true }">
						<td id="project" data-value="${ projectEmployee.project.id }">${ projectEmployee.project.name }</td>
					</c:if>
					<c:if test = "${ showProjects != false }">
